...
using AndroidPCController.Files;
using AndroidPCController.Streaming;
...
public sealed class DeviceSession : IDeviceSession
{
        ...
        ConnectionType connectionType)
    {
        ...
        FileTransfer = new FileTransferService(_adbClient, Serial, new ForwardingLogger<FileTransferService>(logger));
        AppManager = new ApplicationManager(_adbClient, Serial, new ForwardingLogger<ApplicationManager>(logger));
        ScreenStreamer = new ScreenStreamer(_adbClient, Serial, new ForwardingLogger<ScreenStreamer>(logger));
        ScreenRecorder = new AdbScreenRecorder(_adbClient, Serial, new ForwardingLogger<AdbScreenRecorder>(logger));
    }
